...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!permissions.has("CONNECT"))
      return message.reply("No me pude conectar al canal de voz, faltan permisos");
    if (!permissions.has("SPEAK"))
      return message.reply("No pude hablar en este canal de voz, asegúrese de que tengo los permisos adecuados!");
    ...
    if (!serverQueue) {
      try {
        ...
        await channel.leave();
        return message.channel.send(`No pude unirme al canal: ${error}`).catch(console.error);
      }
    }
  }
};
